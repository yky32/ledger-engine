...
public class LedgerAccountBalanceEndpoint {
    ...
    public Result<GetLedgerAccountResponseDto> getBalance(@PathVariable Long id) {
        return R.success(queryAccountUseCase.one(id));
    }

    @GetMapping("/balances")
    public Result<List<GetLedgerAccountBalanceResponseDto>> getAllBalances() {
        return R.success(queryWalletBalanceUseCase.allBalances());
    }

    @GetMapping("/my-balances")
    public Result<List<GetLedgerAccountBalanceResponseDto>> myBalances(@RequestParam String ownerId) {
