...
public class SystemConfigurationUseCase {
    ...
    public SystemDtos.ConfigurationResponse myConfigurations(String target, String scope) {
        return configs.findByTargetAndScope(target, scope)
            .map(DtoMapper::toConfig)
            .orElseThrow(() -> new BizException(AccountErrorResponse.ACC0404, "Config not found: " + target + "/" + scope));
    }

    @Transactional
    public SystemDtos.ConfigurationResponse upsert(String name, String target, String scope, String value) {
