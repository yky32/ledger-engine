...
public class QueryAccountingRuleUseCase {
    ...
    public GetAccountingRuleResponseDto one(Long id) {
        return DtoMapper.toAccountingRule(accountingRuleRepository.findById(id)
    ...
    public Page<GetAccountingRuleResponseDto> list(Pageable pageable) {
        return accountingRuleRepository.findAll(Pageables.toZeroBased(pageable)).map(DtoMapper::toAccountingRule);
    }
}
